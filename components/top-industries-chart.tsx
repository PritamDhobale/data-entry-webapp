...
const COLORS = ["#A7DCF7", "#FFD38D", "#B6E3FF", "#FAC086", "#DAAAE4"];
...
export function TopIndustriesChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch("/api/dashboard/top-industries", { cache: "no-store" });
        const json = await res.json();
        if (!json.success) throw new Error(json.error || "Failed to load industries chart");
        ...
        console.error("Top Industries chart error:", err);
      }
    };
    fetchData();
  }, []);
